...
void filterVenue(value){
 ...
 && int.parse(hrsf_input)+(int.parse(mnsf_input)/60)<=int.parse(hrstrt_input)+(int.parse(mnstrt_input)/60) 
           || int.parse(hrst_input)+(int.parse(mnst_input)/60) > int.parse(hrsfrt_input)+(int.parse(mnsfrt_input)/60) 
           && int.parse(hrst_input)+(int.parse(mnst_input)/60)<=int.parse(hrstrt_input)+(int.parse(mnstrt_input)/60))

|| (int.parse(hrsf_input)+(int.parse(mnsf_input)/60) <= int.parse(hrsfrt_input)+(int.parse(mnsfrt_input)/60) 
 && int.parse(hrsfrt_input)+(int.parse(mnstrt_input)/60)<int.parse(hrst_input)+(int.parse(mnst_input)/60) 
           || int.parse(hrstrt_input)+(int.parse(mnstrt_input)/60) >= int.parse(hrst_input)+(int.parse(mnst_input)/60) 
           && int.parse(hrstrt_input)+(int.parse(mnstrt_input)/60)<int.parse(hrsf_input)+(int.parse(mnsf_input)/60))
    ...
    //apa
    if(error){
  ScaffoldMessenger.of(context).showSnackBar( const SnackBar(content: Text("Time Error")));
    }
    }
  ...
  setState(() {
    ...
    return Scaffold(
      ...
      body:Padding(
        ...
        child: coll==null || coll!.isEmpty? Center(child: Text("No Favourate Available"),): Column(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
             
                coll!=null && coll!.isNotEmpty? VDescription(college: coll![0], total: '${venues.length}',):
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          onTap: () {
           Navigator.push(context, MaterialPageRoute(builder: (context) => SpecificVenue(venue: result, college: collg,)));
          },
          leading: Icon(Icons.class_),
          title: Text(result),
